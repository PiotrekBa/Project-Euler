import java.math.BigInteger;

public class Problem56 {

    /*
    Problem 56 - Powerful digit sum

    A googol (10^100) is a massive number:
    one followed by one-hundred zeros;
    100^100 is almost unimaginably large: one followed by
    two-hundred zeros.
    Despite their size, the sum of the digits in each number is
    only 1.

    Considering natural numbers of the form, a^b,
    where a, b < 100, what is the maximum digital sum?
     */

    public static void main(String[] args) {

        int maxSum = 0;

        for (int i = 1; i < 100; i++) {
            BigInteger a = BigInteger.valueOf(i);
            for (int j = 1; j < 100; j++) {
                BigInteger result = a.pow(j);
                int sum = sumOfDigit(result);
                if (sum > maxSum) {
                    maxSum = sum;
                }
            }
        }
        System.out.println(maxSum);

    }

    private static int sumOfDigit(BigInteger a) {
        String s = a.toString();

        int sum = 0;

        for (int i = 0; i < s.length(); i++) {
            sum += Integer.parseInt("" + s.charAt(i));
        }

        return sum;
    }
}
